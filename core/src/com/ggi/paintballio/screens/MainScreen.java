...
public class MainScreen implements Screen, InputProcessor{
	...
	public MainScreen(PBall pb) {
		...
		if(pb.lastName.length()>0){u=pb.lastName; userField.setText(u);}
	...
	//Render loading
	if(pb.isLoading){
	pic.draw(new TextureRegion(pb.assets.get("loading.png", Texture.class)), pb.w/2-(pb.gridSize), pb.h/8-(pb.gridSize), pb.gridSize, pb.gridSize, 2*pb.gridSize, 2*pb.gridSize, 1, 1, rot , true);
	}
	
	if(pb.error){
		pb.isLoading=false;
		layout = new GlyphLayout(pb.mediumFnt,"Error connecting to server. Please make sure your app is up to date.");
		pb.mediumFnt.setColor(1f, .3f, .3f, 1);
		pb.mediumFnt.draw(pic,"Error connecting to server. Please make sure your app is up to date.",pb.w/2-layout.width/2,.15f*pb.h+layout.height/2);
	}
	//Button
	play.draw(pic, 1);
	pic.end();
