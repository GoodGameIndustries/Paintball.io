...
public class LoadScreen implements Screen{
	...
	public void render(float delta) {
		...
		if(pb.assets.isLoaded("loading.png")){
			if(loading==null){loading = pb.assets.get("loading.png");}
			if(grid==null){grid = pb.assets.get("grid.png");}
		//RenderBG
			pic.begin();
		...
		//Render loading
		pic.draw(new TextureRegion(loading), pb.w/2-(pb.gridSize), pb.h/4-(pb.gridSize), pb.gridSize, pb.gridSize, 2*pb.gridSize, 2*pb.gridSize, 1, 1, rot, true);
